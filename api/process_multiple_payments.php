...
try {
    ...
    // Валидация
    if (empty($orderIds) || $amount <= 0 || empty($cardNumber) || empty($cardholder) || empty($cardExpiry) || empty($cardCvc)) {
    ...
    $orderCheck = $conn->prepare("
        ...
        WHERE o.order_id IN ($placeholders) AND o.client_id = ? AND o.order_status = 'completed'
    ");
    if (!$orderCheck) {
        throw new Exception('Ошибка подготовки запроса: ' . $conn->error);
    }
    $orderCheck->bind_param($types . 'i', ...[...$orderIds, $clientId]);
    if (!$orderCheck->execute()) {
        throw new Exception('Ошибка выполнения запроса: ' . $orderCheck->error);
    }
    $orders = $orderCheck->get_result()->fetch_all(MYSQLI_ASSOC);
    $orderCheck->close();
    
    if (count($orders) !== count($orderIds)) {
        throw new Exception("Некоторые заказы не найдены");
    }
    
    // Проверка суммы и статуса оплаты
    $totalAmount = 0;
    foreach ($orders as $order) {
        if ($order['payment_method'] && $order['payment_method'] !== 'pending') {
            throw new Exception("Заказ {$order['order_id']} уже оплачен");
        }
        $totalAmount += $order['sale_price'];
    }
    
    if (round($amount, 2) != round($totalAmount, 2)) {
        throw new Exception("Неверная сумма платежа: ожидается $" . number_format($totalAmount, 2));
    }
    ...
    foreach ($orders as $order) {
        ...
        $message = "Ваш платеж на сумму $" . number_format($order['sale_price'], 2) . " за {$car['make']} {$car['model']} прошел успешно. Номер заказа: $orderId";
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Все заказы успешно оплачены'
    ...
    error_log("Multiple payments error: " . $e->getMessage() . " in " . __FILE__ . " at line " . __LINE__);
    if (ob_get_level()) {
        ob_clean();
    }
    header('Content-Type: application/json; charset=utf-8');
